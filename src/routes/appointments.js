...
async function nextAppointmentId() {
  const last = await db.one('SELECT appointment_id FROM appointments ORDER BY id DESC LIMIT 1');
...
async function loadCapacityPools() {
  const pools = await db.rows('SELECT * FROM capacity_pools');
...
async function loadOrderedServices(serviceIds) {
  if (!serviceIds.length) return [];
  const placeholders = serviceIds.map(() => '?').join(',');
  const rows = await db.rows(`SELECT * FROM services WHERE id IN (${placeholders})`, serviceIds);
...
async function loadDayBookings(date) {
  return db.rows(
    `SELECT a.appointment_time,a.duration_mins,a.staff_id,s.capacity_pool_id
     ...
     WHERE a.appointment_date=:date AND a.status NOT IN ('cancelled','no_show')`,
    { date },
  );
}

async function getSlots(date, serviceIds, staffId) {
  const services = await loadOrderedServices(serviceIds);
...
async function insertAppointmentGroup(body, status, source) {
  ...
  const staff = staffId ? await db.one('SELECT * FROM staff WHERE id=:staffId', { staffId }) : null;
  if (staffId) {
    const restricted = await db.one(`SELECT COUNT(DISTINCT service_id) count FROM service_staff WHERE service_id IN (${serviceIds.map(() => '?').join(',')})`, serviceIds);
    const matched = await db.one(`SELECT COUNT(DISTINCT service_id) count FROM service_staff WHERE staff_id=? AND service_id IN (${serviceIds.map(() => '?').join(',')})`, [staffId, ...serviceIds]);
  ...
  for (const service of services) {
    ...
    const appointment = {
      appointment_id: await nextAppointmentId(), booking_token: crypto.randomBytes(18).toString('base64url'), group_token: groupToken,
    ...
    if (source === 'online') {
      const existing = await db.one('SELECT id FROM customers WHERE mobile=:mobile AND archived=0', { mobile: appointment.customer_mobile });
      if (existing) appointment.customer_id = existing.id;
    }
    await db.rows(`INSERT INTO appointments(appointment_id,booking_token,group_token,customer_id,customer_name,customer_mobile,customer_email,
      ...
      VALUES(:appointment_id,:booking_token,:group_token,:customer_id,:customer_name,:customer_mobile,:customer_email,:service_id,:service_name,
...
module.exports=app=>{
  app.get('/appointments',auth,asyncRoute(async(req,res)=>{
    ...
    let sql='SELECT * FROM appointments WHERE 1=1';const params={};
    ...
    const todaySql="SELECT * FROM appointments WHERE appointment_date=:today AND status NOT IN ('cancelled','no_show')"+(req.user?.role==='team'?' AND staff_id=:scopeStaffId':'')+' ORDER BY appointment_time';
    const[rows,today_list]=await Promise.all([db.rows(sql,params),db.rows(todaySql,{today:isoDate(),scopeStaffId:Number(req.user?.staff_id||0)})]);
    res.render('appointments.html',{rows,today_list,status_f,date_from,date_to,q,today:isoDate()});
  }));
  app.get('/appointments/new',auth,asyncRoute(async(_req,res)=>{const[services,staff,customers]=await Promise.all([db.rows("SELECT s.*,GROUP_CONCAT(ss.staff_id ORDER BY ss.staff_id) staff_ids FROM services s LEFT JOIN service_staff ss ON ss.service_id=s.id WHERE s.archived=0 AND s.status='Active' GROUP BY s.id ORDER BY s.name"),db.rows("SELECT * FROM staff WHERE archived=0 AND status='Active' ORDER BY name"),db.rows('SELECT id,customer_id,name,mobile,email FROM customers WHERE archived=0 ORDER BY name')]);res.render('appointment_form.html',{services,staff,customers});}));
  app.post('/appointments/new',auth,asyncRoute(async(req,res)=>{
    const appointments=await insertAppointmentGroup(req.body,'confirmed','walkin');
  ...
  app.get('/appointments/:aid',auth,asyncRoute(async(req,res)=>{const appt=await db.one('SELECT * FROM appointments WHERE id=:aid',{aid:Number(req.params.aid)});if(!appt)return res.status(404).send('Appointment not found');if(req.user?.role==='team'&&Number(appt.staff_id)!==Number(req.user.staff_id||0))return res.status(403).render('access_denied.html');const group=appt.group_token?await db.rows('SELECT * FROM appointments WHERE group_token=:token AND id!=:aid ORDER BY appointment_time',{token:appt.group_token,aid:appt.id}):[];res.render('appointment_detail.html',{appt,group,cfg:req.settings});}));
  app.post('/appointments/:aid/status',auth,asyncRoute(async(req,res)=>{const status=req.body.status;if(!['pending','confirmed','completed','cancelled','no_show'].includes(status))return res.status(400).send('Invalid status');const aid=Number(req.params.aid);await db.rows('UPDATE appointments SET status=:status,cancel_reason=:reason WHERE id=:aid',{status,reason:req.body.cancel_reason||'',aid});const appointment=await db.one('SELECT * FROM appointments WHERE id=:aid',{aid});if(['confirmed','cancelled'].includes(status)){const type=status==='confirmed'?'confirmation':'cancellation',results=await sendBookingNotifications(req.settings,appointment,type),failures=results.filter(r=>!r.ok).map(r=>`${r.channel}: ${r.message}`);if(failures.length)req.flash('error',`Status saved, but notification failed: ${failures.join('; ')}`);}req.flash('success',`Appointment marked as ${status.replaceAll('_',' ')}.`);res.redirect(`/appointments/${aid}`);}));
  app.post('/appointments/:aid/send-reminder',auth,asyncRoute(async(req,res)=>{const aid=Number(req.params.aid),appointment=await db.one('SELECT * FROM appointments WHERE id=:aid',{aid});if(!appointment)return res.status(404).send('Appointment not found');const results=await sendBookingNotifications(req.settings,appointment,'reminder'),ok=results.filter(r=>r.ok).length;if(ok)await db.rows('UPDATE appointments SET reminder_sent=1 WHERE id=:aid',{aid});const failures=results.filter(r=>!r.ok).map(r=>`${r.channel}: ${r.message}`).join('; ');req.flash(ok?'success':'error',`Reminder sent (${ok}/${results.length} channels OK).${failures?` ${failures}`:''}`);res.redirect(`/appointments/${aid}`);}));

  app.get('/book',asyncRoute(async(_req,res)=>{const[services,staff]=await Promise.all([db.rows("SELECT * FROM services WHERE archived=0 AND status='Active' ORDER BY popular DESC,name"),db.rows("SELECT * FROM staff WHERE archived=0 AND status='Active' ORDER BY name")]);res.render('book.html',{services,staff,cfg:res.locals.cfg});}));
  app.post('/book',asyncRoute(async(req,res)=>{const appointments=await insertAppointmentGroup(req.body,'pending','online');res.redirect(`/book/success/${appointments.map(a=>a.booking_token).join('+')}`);}));
  app.get('/book/success/:tokens',asyncRoute(async(req,res)=>{const tokens=req.params.tokens.split('+');const placeholders=tokens.map(()=>'?').join(',');const appts=await db.rows(`SELECT * FROM appointments WHERE booking_token IN (${placeholders}) ORDER BY appointment_time`,tokens);if(!appts.length)return res.status(404).send('Booking not found');res.render('book_success.html',{appts,total:appts.reduce((sum,a)=>sum+Number(a.amount||0),0),cfg:req.settings});}));
  app.get('/book/cancel/:token',asyncRoute(async(req,res)=>{const appt=await db.one('SELECT * FROM appointments WHERE booking_token=:token',{token:req.params.token});if(!appt)return res.status(404).send('Booking not found');res.render('book_cancel.html',{appt,cfg:req.settings,already_done:['cancelled','completed','no_show'].includes(appt.status)});}));
  app.post('/book/cancel/:token',asyncRoute(async(req,res)=>{const appt=await db.one('SELECT * FROM appointments WHERE booking_token=:token',{token:req.params.token});if(!appt)return res.status(404).send('Booking not found');if(!['cancelled','completed','no_show'].includes(appt.status)){await db.rows("UPDATE appointments SET status='cancelled',cancel_reason='Cancelled by customer' WHERE booking_token=:token",{token:req.params.token});await sendBookingNotifications(req.settings,appt,'cancellation');}res.redirect(`/book/cancel/${req.params.token}`);}));
  app.get('/api/slots',asyncRoute(async(req,res)=>{const date=req.query.date,serviceIds=asIdList(req.query.service_id),staffId=req.query.staff_id?Number(req.query.staff_id):null;if(!/^\d{4}-\d{2}-\d{2}$/.test(date||'')||!serviceIds.length)return res.json([]);res.json(await getSlots(date,serviceIds,staffId));}));
  app.post('/tasks/send-reminders',asyncRoute(async(req,res)=>{
    ...
    const appointments=await db.rows("SELECT * FROM appointments WHERE appointment_date=:date AND status IN ('pending','confirmed') AND reminder_sent=0",{date:isoDate(tomorrow)});
    let sent=0,failed=0;
    for(const appointment of appointments){const results=await sendBookingNotifications(req.settings,appointment,'reminder');if(results.some(result=>result.ok)){await db.rows('UPDATE appointments SET reminder_sent=1 WHERE id=:id',{id:appointment.id});sent++;}else failed++;}
