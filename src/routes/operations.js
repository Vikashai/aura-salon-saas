...
async function saveServiceStaff(serviceId, staffIds) {
  const ids = [...new Set(staffIds.map(Number).filter(Number.isInteger))];
  await db.transaction(async connection => {
    await connection.execute('DELETE FROM service_staff WHERE service_id=?', [serviceId]);
    for (const staffId of ids) await connection.execute('INSERT IGNORE INTO service_staff(service_id,staff_id) VALUES(?,?)', [serviceId, staffId]);
  });
}
async function saveStaffServices(staffId, serviceIds) {
  const ids = [...new Set(serviceIds.map(Number).filter(Number.isInteger))];
  await db.transaction(async connection => {
    await connection.execute('DELETE FROM service_staff WHERE staff_id=?', [staffId]);
    for (const serviceId of ids) await connection.execute('INSERT IGNORE INTO service_staff(service_id,staff_id) VALUES(?,?)', [serviceId, staffId]);
  });
}
...
module.exports = app => {
  app.get('/manage/:module', auth, asyncRoute(async (req, res) => {
    ...
    const archived = ['services','staff','products','packages'].includes(table) ? ' WHERE archived=0' : '';
    const isServices = table === 'services';
    const rows = await db.rows(isServices
      ? `SELECT services.*, capacity_pools.name capacity_pool_name FROM services LEFT JOIN capacity_pools ON capacity_pools.id=services.capacity_pool_id${archived} ORDER BY services.id DESC`
      : `SELECT * FROM ${table}${archived} ORDER BY id DESC`);
    const categories = [...new Set(rows.map(row=>row.category).filter(Boolean))].sort();
    const capacity_pools = isServices ? await db.rows('SELECT * FROM capacity_pools ORDER BY is_default DESC,name') : [];
    const staff_options = (isServices || table === 'expenses') ? await db.rows("SELECT id,name,role,fixed_salary FROM staff WHERE archived=0 AND status='Active' ORDER BY name") : [];
    const service_options = table === 'staff' ? await db.rows("SELECT id,name,category FROM services WHERE archived=0 AND status='Active' ORDER BY category,name") : [];
  ...
  app.post('/manage/:module', auth, asyncRoute(async (req, res) => {
    ...
    if (table === 'expenses') {
      ...
      if (category === 'Payroll') {
        ...
        for(const staffId of staffIds){const person=await db.one('SELECT id,name FROM staff WHERE id=:id AND archived=0',{id:staffId}),amount=Number(req.body[`payroll_amount_${staffId}`]||0);if(!person||amount<=0)continue;await db.rows('INSERT INTO expenses(expense_date,category,subcategory,employee_name,expense_group,amount,payment_mode,paid_to,reference_no,period_start,period_end,due_date,notes) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',[expenseDate,'Payroll','Salary',person.name,group,amount,paymentMode,person.name,referenceNo,periodStart,periodEnd,dueDate,notes]);created++;}
      ...
      await db.rows('INSERT INTO expenses(expense_date,category,subcategory,employee_name,expense_group,amount,payment_mode,paid_to,reference_no,period_start,period_end,due_date,notes) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',[expenseDate,category,String(req.body.subcategory||'').trim()||null,String(req.body.employee_name||'').trim()||null,null,amount,paymentMode,String(req.body.paid_to||'').trim()||null,referenceNo,periodStart,periodEnd,dueDate,notes]);
      req.flash('success','Expense added successfully.');return res.redirect('/manage/expenses');
    }
    const placeholders = fields.map(() => '?').join(',');
    const result = await db.rows(`INSERT INTO ${table} (${fields.map(field=>`\`${field}\``).join(',')}) VALUES (${placeholders})`, fields.map(field => req.body[field] || null));
    if (table === 'services') await saveServiceStaff(result.insertId, values(req.body, 'staff_ids'));
    if (table === 'staff') await saveStaffServices(result.insertId, values(req.body, 'service_ids'));
  ...
  app.post('/manage/:module/import', auth, upload.single('spreadsheet'), asyncRoute(async(req,res)=>{
    ...
    for(const raw of records){const source=Object.fromEntries(Object.entries(raw).map(([key,value])=>[normalize(key),value]));const values=fields.map(field=>source[field]??'');if(!values[0]){skipped++;continue;}
      await db.rows(`INSERT INTO ${table} (${fields.map(field=>`\`${field}\``).join(',')}) VALUES (${fields.map(()=>'?').join(',')})`,values.map(value=>value===''?null:value));imported++;}
  ...
  app.post('/manage/:module/bulk', auth, asyncRoute(async(req,res)=>{
    ...
    if(action==='active'&&['services','staff','products','packages'].includes(table))await db.rows(`UPDATE ${table} SET status='Active' WHERE id IN (${placeholders})`,ids);
    else if(action==='inactive'&&['services','staff','products','packages'].includes(table))await db.rows(`UPDATE ${table} SET status='Inactive' WHERE id IN (${placeholders})`,ids);
    else if(action==='archive'&&['services','staff','products','packages'].includes(table))await db.rows(`UPDATE ${table} SET archived=1 WHERE id IN (${placeholders})`,ids);
  ...
  app.get('/manage/:module/:id/edit', auth, asyncRoute(async(req,res)=>{
    const definition=MODULES[req.params.module];if(!definition)return res.status(404).send('Not found');const [table,title,singular,fields,labels]=definition,row=await db.one(`SELECT * FROM ${table} WHERE id=:id`,{id:Number(req.params.id)});if(!row)return res.status(404).send('Record not found');
    const capacity_pools=table==='services'?await db.rows('SELECT * FROM capacity_pools ORDER BY is_default DESC,name'):[];
    const staff_options=table==='services'?await db.rows("SELECT id,name,role FROM staff WHERE archived=0 AND status='Active' ORDER BY name"):[];
    const service_options=table==='staff'?await db.rows("SELECT id,name,category FROM services WHERE archived=0 AND status='Active' ORDER BY category,name"):[];
    const selected_staff_ids=table==='services'?(await db.rows('SELECT staff_id FROM service_staff WHERE service_id=:id',{id:Number(req.params.id)})).map(item=>item.staff_id):[];
    const selected_service_ids=table==='staff'?(await db.rows('SELECT service_id FROM service_staff WHERE staff_id=:id',{id:Number(req.params.id)})).map(item=>item.service_id):[];
    const allCategories=table==='services'?await db.rows("SELECT DISTINCT category FROM services WHERE archived=0 AND category IS NOT NULL ORDER BY category"):[];
  ...
  app.post('/manage/:module/:id/edit', auth, asyncRoute(async(req,res)=>{
    const definition=MODULES[req.params.module];if(!definition)return res.status(404).send('Not found');const [table,,singular,fields]=definition,id=Number(req.params.id);if(table==='services')req.body.category=serviceCategory(req.body.gender,req.body.category_name==='__new__'?req.body.custom_category_name:req.body.category_name);const assignments=fields.map(field=>`\`${field}\`=?`).join(',');await db.rows(`UPDATE ${table} SET ${assignments} WHERE id=?`,[...fields.map(field=>req.body[field]||null),id]);if(table==='services')await saveServiceStaff(id,values(req.body,'staff_ids'));if(table==='staff')await saveStaffServices(id,values(req.body,'service_ids'));req.flash('success',`${singular} updated.`);res.redirect(`/manage/${req.params.module}`);
  }));

  app.get('/reports', auth, asyncRoute(async (req, res) => {
    const defaultStart = new Date(); defaultStart.setDate(defaultStart.getDate()-30);
    const start = req.query.start || isoDate(defaultStart), end = req.query.end || isoDate();
    const [sales, expenseRow] = await Promise.all([
      db.rows('SELECT s.*,c.name customer FROM sales s LEFT JOIN customers c ON c.id=s.customer_id WHERE invoice_date BETWEEN :start AND :end AND cancelled=0 ORDER BY invoice_date DESC',{start,end}),
      db.one('SELECT COALESCE(SUM(amount),0) value FROM expenses WHERE expense_date BETWEEN :start AND :end',{start,end}),
    ]);
  ...
  app.get('/greetings', auth, asyncRoute(async (_req,res) => {
    const today = new Date(), todayMd = isoDate(today).slice(5);
    ...
    const [birthdays_today,anniversaries_today,upcoming_birthdays,upcoming_anniversaries,settingsRows]=await Promise.all([
      db.rows("SELECT * FROM customers WHERE DATE_FORMAT(dob,'%m-%d')=? AND status='Active' ORDER BY name",[todayMd]),
      db.rows("SELECT * FROM customers WHERE DATE_FORMAT(anniversary,'%m-%d')=? AND status='Active' ORDER BY name",[todayMd]),
      db.rows(`SELECT * FROM customers WHERE DATE_FORMAT(dob,'%m-%d') IN (${placeholders}) AND status='Active' ORDER BY DATE_FORMAT(dob,'%m-%d')`,upcoming),
      db.rows(`SELECT * FROM customers WHERE DATE_FORMAT(anniversary,'%m-%d') IN (${placeholders}) AND status='Active' ORDER BY DATE_FORMAT(anniversary,'%m-%d')`,upcoming),
      db.rows('SELECT `key`,`value` FROM settings'),
    ]);
  ...
  app.post('/greetings/send',auth,asyncRoute(async(req,res)=>{
    const customer=await db.one('SELECT * FROM customers WHERE id=:id',{id:Number(req.body.customer_id)}),type=req.body.msg_type;
  ...
  app.post('/greetings/templates',auth,asyncRoute(async(req,res)=>{
    for(const key of ['msg_welcome','msg_birthday','msg_anniversary']) await db.rows('INSERT INTO settings (`key`,`value`) VALUES (:key,:value) ON DUPLICATE KEY UPDATE `value`=VALUES(`value`)',{key,value:req.body[key]||''});
    req.flash('success','Message templates saved.');res.redirect('/greetings');
  }));

  app.get('/loyalty',auth,asyncRoute(async(req,res)=>{
    const statQueries={pts_in_circulation:'SELECT COALESCE(SUM(loyalty_points),0) value FROM customers',pts_awarded:'SELECT COALESCE(SUM(points),0) value FROM loyalty_transactions WHERE points>0',pts_redeemed:"SELECT ABS(COALESCE(SUM(points),0)) value FROM loyalty_transactions WHERE type='redeem'",total_discount:'SELECT COALESCE(SUM(loyalty_discount+referral_discount+referral_credit_used),0) value FROM sales',total_referrals:'SELECT COUNT(*) value FROM customers WHERE referred_by_id IS NOT NULL',active_members:'SELECT COUNT(*) value FROM customers WHERE loyalty_points>0 OR referral_credit>0',total_customers:'SELECT COUNT(*) value FROM customers'};
    const stats={};for(const[key,sql]of Object.entries(statQueries))stats[key]=Number((await db.one(sql)).value);
    const [leaderboard,recent_txns,referral_pairs]=await Promise.all([
      db.rows('SELECT id,name,mobile,loyalty_points FROM customers WHERE loyalty_points>0 ORDER BY loyalty_points DESC LIMIT 10'),
      db.rows('SELECT lt.*,c.name customer_name,c.id cust_id FROM loyalty_transactions lt JOIN customers c ON lt.customer_id=c.id ORDER BY lt.id DESC LIMIT 50'),
      db.rows('SELECT r.id referee_id,r.name referee_name,r.mobile referee_mobile,r.loyalty_points referee_pts,ref.name referrer_name,ref.mobile referrer_mobile,ref.referral_credit referrer_credit FROM customers r JOIN customers ref ON r.referred_by_id=ref.id ORDER BY r.id DESC LIMIT 30'),
    ]);
  ...
  app.post('/loyalty/settings',auth,asyncRoute(async(req,res)=>{
    ...
    for(const key of keys)await db.rows('INSERT INTO settings (`key`,`value`) VALUES (:key,:value) ON DUPLICATE KEY UPDATE `value`=VALUES(`value`)',{key,value:req.body[key]||''});
  ...
  app.post('/customers/:cid/loyalty/adjust',auth,asyncRoute(async(req,res)=>{
    const cid=Number(req.params.cid),points=Number.parseInt(req.body.points||0,10),customer=await db.one('SELECT loyalty_points FROM customers WHERE id=:cid',{cid});
    ...
    await db.transaction(connection=>awardPoints(connection,cid,points,'manual',String(req.body.reason||'').trim()||'Manual adjustment'));
  ...
  app.get('/api/loyalty/:cid',auth,asyncRoute(async(req,res)=>{
    const customer=await db.one('SELECT loyalty_points FROM customers WHERE id=:cid',{cid:Number(req.params.cid)}),cfg=loyaltyConfig(req.settings),balance=Number(customer?.loyalty_points||0),amount=Number(req.query.amount||0);
  ...
  app.get('/settings',auth,asyncRoute(async(_req,res)=>{const data={...res.locals.cfg};data.meta_token_configured=Boolean(data.meta_whatsapp_token);data.twilio_token_configured=Boolean(data.twilio_token);data.smtp_pass_configured=Boolean(data.smtp_pass);data.meta_whatsapp_token='';data.twilio_token='';data.smtp_pass='';data.get=(key,fallback='')=>data[key]??fallback;const capacity_pools=await db.rows('SELECT * FROM capacity_pools ORDER BY is_default DESC,name');res.render('settings.html',{data,capacity_pools});}));
  app.post('/settings/capacity-pools/add',auth,asyncRoute(async(req,res)=>{const name=String(req.body.new_pool_name||'').trim(),seats=Number.parseInt(req.body.new_pool_seats,10);if(!name||!Number.isInteger(seats)||seats<1){req.flash('error','Enter a pool name and a seat count of at least 1.');return res.redirect('/settings#capacity');}await db.rows('INSERT INTO capacity_pools (name,seats) VALUES (:name,:seats)',{name,seats});req.flash('success',`Capacity pool "${name}" added.`);res.redirect('/settings#capacity');}));
  app.post('/settings/capacity-pools/:id',auth,asyncRoute(async(req,res)=>{const id=Number(req.params.id),name=String(req.body[`pool_name_${id}`]||'').trim(),seats=Number.parseInt(req.body[`pool_seats_${id}`],10);if(!name||!Number.isInteger(seats)||seats<1){req.flash('error','Enter a pool name and a seat count of at least 1.');return res.redirect('/settings#capacity');}await db.rows('UPDATE capacity_pools SET name=:name,seats=:seats WHERE id=:id',{name,seats,id});req.flash('success','Capacity pool updated.');res.redirect('/settings#capacity');}));
  app.post('/settings/capacity-pools/:id/delete',auth,asyncRoute(async(req,res)=>{const id=Number(req.params.id),pool=await db.one('SELECT is_default FROM capacity_pools WHERE id=:id',{id});if(!pool){req.flash('error','Capacity pool not found.');return res.redirect('/settings#capacity');}if(pool.is_default){req.flash('error','The default pool cannot be deleted.');return res.redirect('/settings#capacity');}await db.rows('UPDATE services SET capacity_pool_id=NULL WHERE capacity_pool_id=:id',{id});await db.rows('DELETE FROM capacity_pools WHERE id=:id',{id});req.flash('success','Capacity pool removed. Services using it now use the default pool.');res.redirect('/settings#capacity');}));
  app.post('/settings',auth,asyncRoute(async(req,res)=>{for(const key of SETTINGS_KEYS){const value=req.body[key]||'';if(['meta_whatsapp_token','twilio_token','smtp_pass'].includes(key)&&!value)continue;await db.rows('INSERT INTO settings (`key`,`value`) VALUES (:key,:value) ON DUPLICATE KEY UPDATE `value`=VALUES(`value`)',{key,value});}req.flash('success','Settings updated.');res.redirect('/settings');}));
